...
static gboolean 
example_hello_svc_handler_establish_context (helloSvcIf *iface, return_ec ** _return, const DWORD dwScope, GError **error) {
  THRIFT_UNUSED_VAR (iface);
  THRIFT_UNUSED_VAR (error);
...
example_hello_svc_handler_class_init (ExampleHelloSvcHandlerClass *klass)
{
  ...
  hello_svc_handler_class->establish_context = example_hello_svc_handler_establish_context;
}
